...
export default function RoomTypeFormModal({ show, onClose, onSaved }) {
  const [form, setForm] = useState({ name: 'STANDARD', bedConfiguration: '', maxOccupancy: 2, baseRate: 2000, amenitiesList: '' })
  ...
  const submit = async (e) => {
    ...
    try { await createRoomType({ ...form, maxOccupancy: Number(form.maxOccupancy), baseRate: Number(form.baseRate) }); onSaved() }
    ...
    <Modal show={show} onHide={onClose}>
      ...
      <Form onSubmit={submit}>
        <Modal.Body>
          ...
          <Form.Group className="mb-3"><Form.Label>Bed Configuration</Form.Label><Form.Control value={form.bedConfiguration} onChange={e => set('bedConfiguration', e.target.value)} placeholder="e.g., 1 King + 1 Sofa" /></Form.Group>
